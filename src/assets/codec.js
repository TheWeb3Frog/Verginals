'use strict';
// Verge Assets: the OP_RETURN wire format (ASSETS-SPEC-v0 §4).
//
...
//   { type: 'edicts',     edicts: [{ assetRef, amount, output }] }
//   { type: 'mint',       assetRef, proofIndex|null }
//   { type: 'checkpoint', height, root: Buffer(32) }
//
// Edicts carry ABSOLUTE asset references once decoded; the delta encoding of spec §4.1 is an
// on-the-wire detail and never leaks into the state machine.

// An edict stream and the control messages share the first body byte, so the smallest asset
...
const MIN_ASSET_REF = 3;

/** Encode a transfer message. Edicts are sorted by assetRef, then delta-encoded. */
function encodeEdicts(edicts) {
  if (!Array.isArray(edicts) || edicts.length === 0) throw new Error('at least one edict is required');
  const sorted = edicts.slice().sort((a, b) => a.assetRef - b.assetRef);
  if (sorted[0].assetRef < MIN_ASSET_REF) {
    throw new Error(`asset references below ${MIN_ASSET_REF} are reserved (they would decode as a control message)`);
  }
  const parts = [];
  let prev = 0;
  sorted.forEach((e, i) => {
    if (!Number.isInteger(e.assetRef) || e.assetRef < 0) throw new Error('assetRef must be a non-negative integer');
    if (!Number.isInteger(e.output) || e.output < 0) throw new Error('output must be a non-negative integer');
    const delta = e.assetRef - prev;
    prev = e.assetRef;
...
function encodeMint(assetRef, proofIndex = null) {
  const parts = [encodeVarint(OP_MINT), encodeVarint(assetRef)];
...
function decode(payload) {
  ...
  // The first varint of an edict stream is the lowest absolute asset reference, which is why
  // references below MIN_ASSET_REF are reserved and refused at encode time.
...
function decodeMint(body) {
  ...
  return { type: 'mint', assetRef: a.value, proofIndex };
}
...
module.exports = {
  MAGIC, VERSION, MAX_PAYLOAD, OP_MINT, OP_CHECKPOINT, MIN_ASSET_REF,
