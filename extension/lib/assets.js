// Verge Assets in the browser wallet (ESM, MV3-safe). Mirrors the rules of src/assets/*, checked
// against them in extension/test-assets.mjs.
//
// A wallet cannot index the whole chain, and it must not have to trust whoever indexes it for it.
// That is exactly what checkpoints are for (ASSETS-SPEC-v0 §8): an indexer hands over a balance and
...
//
// The other half of this file is the safety rule that matters more than any feature: an asset-
...
export function decodeMessage(payload) {
  ...
  if (body[0] === OP_MINT) {
    ...
    return a ? { type: 'mint', assetRef: a.value } : null;
  }
  ...
  while (off < body.length) {
    ...
    ref += d.value;
    edicts.push({ assetRef: ref, amount: amt.value, output: out.value });
    off = flags.next;
...
export async function verifyBalance(entry, path, root) {
  ...
  let node = await verge.sha256(enc.encode(`${entry.outpoint}|${entry.assetRef}|${entry.amount}`));
 ...
 * Returns { balances: Map(outpoint -> {assetRef: amount}), rejected }.
 */
export async function verifiedBalances(answer, root) {
  ...
  for (const item of (answer && answer.entries) || []) {
    ...
    const { outpoint, assetRef, amount } = item.entry;
    const at = balances.get(outpoint) || {};
    at[assetRef] = (at[assetRef] || 0) + amount;
...
/** Does this coin carry any asset? `undefined` means "not known", which counts as yes. */
export function carriesAsset(utxo) {
  if (!utxo || utxo.assets === undefined || utxo.assets === null) return true; // unknown: assume it does
  return Object.keys(utxo.assets).length > 0;
}

/**
 * The coins a plain payment may spend: assets confirmed absent AND inscriptions confirmed absent.
 *
...
export function spendableForPayment(utxos) {
  return (utxos || []).filter((u) => u.inscription === null && u.assets !== undefined
    && u.assets !== null && Object.keys(u.assets).length === 0);
}

/**
 * Coins for a transfer that is deliberately moving `assetRef`: the carriers, plus clean coins for
 * the fee. Throws rather than reaching for another asset's carrier to make up a shortfall.
 */
export function selectForAssetTransfer(utxos, assetRef, amount, { targetValue = 0, fee = 0 } = {}) {
  const holders = (utxos || [])
    .filter((u) => u.assets && u.assets[assetRef] > 0)
    .sort((a, b) => b.assets[assetRef] - a.assets[assetRef]);
  const held = holders.reduce((s, u) => s + u.assets[assetRef], 0);
  const need = amount === 0 ? held : amount;
  if (need <= 0 || held < need) {
    const e = new Error(`insufficient balance for asset ${assetRef}: need ${need}, hold ${held}`);
    e.name = 'InsufficientAsset';
    throw e;
  }
  ...
  for (const u of inputs) {
    for (const [ref, amt] of Object.entries(u.assets || {})) {
      if (Number(ref) === Number(assetRef)) continue;
