// End-to-end on the isolated regtest chain (ASSETS-PLAN phase 3).
//
...
const { scanRange, toIndexerTx, readOpReturn } = require(path.join(ROOT, 'src/assets/scanner'));
...
(async () => {
  console.log('Verge Assets end-to-end on regtest\n');
  ...
  const REF = assetRefOf(startHeight + 1, 1); // pretend the asset was etched here
  const edictMsg = codec.encodeEdicts([{ assetRef: REF, amount: 250000, output: 0 }]);
  ...
  check('the scanner + codec decode the edict back to its exact values',
    decoded && decoded.type === 'edicts' && decoded.edicts[0].assetRef === REF && decoded.edicts[0].amount === 250000,
  ...
  check('a mint message survives the chain', m2 && m2.type === 'mint' && m2.assetRef === REF);
  ...
  // Etch the asset synthetically at the height we referenced, then let the scanner replay the real
  ...
  const etching = { ticker: 'REGT', name: 'Regtest', divisibility: 2, supply: 1000000, premine: 900000,
    terms: { amount: 1000 } };
  const chainState = new AssetState();
  ...
  check('the synthetic etching registered the asset', chainState.assets.get(REF) && chainState.assets.get(REF).ticker === 'REGT');
  ...
  // The mint we broadcast is a real one against a live asset with open terms.
  const mintedSomewhere = [...chainState.entries()].some((e) => e.assetRef === REF && e.amount === 1000);
  ...
  if (first) {
    const proof = proveBalance(chainState, first.outpoint, first.assetRef);
