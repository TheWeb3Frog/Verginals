// Rune transaction builders (RUNES-PLAN §2.2).
...
// Run: node test/runes-builder.test.js
const assert = require('assert');
const { DUST_UNITS, buildTransfer, buildMint, buildCheckpoint, buildEtch } = require('../src/runes/builder');
...
test('the OP_RETURN is always last, so no edict can point at it', () => {
  const plan = buildTransfer([
    { address: 'D1', value: DUST_UNITS, runes: [{ runeRef: REF, amount: 10 }] },
    { address: 'D2', value: DUST_UNITS, runes: [{ runeRef: REF2, amount: 20 }] },
...
test('edict indices match the recipient positions exactly', () => {
  ...
  const byRef = Object.fromEntries(decoded.edicts.map((e) => [e.runeRef, e.output]));
...
test('change sits between the recipients and the OP_RETURN, never shifting an edict index', () => {
  ...
  const without = buildTransfer([{ address: 'D1', value: DUST_UNITS, runes: [{ runeRef: REF, amount: 10 }] }]);
...
test('a rune output below dust is refused', () => {
  assert.throws(
    () => buildTransfer([{ address: 'D1', value: DUST_UNITS - 1, runes: [{ runeRef: REF, amount: 1 }] }]),
    /dust minimum/,
  );
});

test('a transfer with no edict is refused (it would just be an ordinary send)', () => {
  assert.throws(() => buildTransfer([{ address: 'D1', value: DUST_UNITS }]), /no rune edict/);
...
test('the caller is told where the unassigned remainder will land', () => {
  const plan = buildTransfer([{ address: 'D1', value: DUST_UNITS, runes: [{ runeRef: REF, amount: 10 }] }]);
...
test('a mint puts the recipient first and the message last', () => {
  ...
  assert.strictEqual(msg.runeRef, REF);
});
...
test('a mint price above what a node will relay is refused at etch time', () => {
  ...
  // etch a rune nobody could mint with an ordinary wallet, permanently
  const lock = { locktime: 1700000000 + LOCK_SECONDS, pubkey: KEY };
