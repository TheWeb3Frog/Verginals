...
function populated() {
  ...
  applyTx(s, {
    ...
    outputs: [out(), out(), out(), opret(codec.encodeEdicts([
      { assetRef: REF, amount: 250000, output: 1 },
      { assetRef: REF, amount: 250000, output: 2 },
    ]))],
  });
  return s;
}
...
test('an empty state commits to the zero root', () => {
  assert.ok(stateRoot(new AssetState()).equals(Buffer.alloc(32)));
});
...
test('vector 9: a wallet can prove its own balance against the published root', () => {
  ...
  for (const e of s.entries()) {
    const p = proveBalance(s, e.outpoint, e.assetRef);
...
test('odd node counts are carried up, never duplicated (no second-preimage trick)', () => {
  ...
  const three = [
    { outpoint: 'a:0', assetRef: 1, amount: 1 },
    { outpoint: 'b:0', assetRef: 1, amount: 2 },
    { outpoint: 'c:0', assetRef: 1, amount: 3 },
  ];
  const four = three.concat([{ outpoint: 'c:0', assetRef: 1, amount: 3 }]);
  assert.ok(!buildTree(three).root.equals(buildTree(four).root));
});

test('every leaf of a 1..12 leaf tree proves correctly', () => {
  for (let n = 1; n <= 12; n++) {
    const entries = Array.from({ length: n }, (_, i) => ({ outpoint: `t${i}:0`, assetRef: 5, amount: i + 1 }));
...
console.log('\nassets checkpoint: ' + passed + ' passed');
