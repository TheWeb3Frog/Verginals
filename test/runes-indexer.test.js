// Verge Runes state machine (RUNES-SPEC-v0 §12 test vectors).
// Run: node test/runes-indexer.test.js
const assert = require('assert');
const crypto = require('crypto');
const { RuneState, applyTx: rawApplyTx, index: rawIndex, runeRefOf, outpoint } = require('../src/runes/indexer');
...
const index = (txs, o) => rawIndex(txs, Object.assign({}, RELAXED, o));
...
test('an etching registers the rune and premines to the first output', () => {
  const s = etched();
  assert.strictEqual(s.runes.get(REF).ticker, 'FROG');
...
test('an invalid etching (bad ticker, premine over supply) is refused', () => {
  for (const bad of [{ ticker: 'lower case!' }, { premine: 999999999 }, { divisibility: 9 }]) {
    ...
    assert.strictEqual(s.runes.size, 0, JSON.stringify(bad));
  }
});
...
test('an edict moves the stated amount and leaves the rest to the remainder output', () => {
  ...
  applyTx(s, {
    ...
    outputs: [out(), out(), opret(codec.encodeEdicts([{ runeRef: REF, amount: 30000, output: 1 }]))],
  });
...
test('vector 3: amount 0 moves the entire pooled balance', () => {
  ...
  applyTx(s, {
    ...
    inputs: [{ txid: 'etch', vout: 0 }],
    outputs: [out(), out(), opret(codec.encodeEdicts([{ runeRef: REF, amount: 0, output: 1 }]))],
  });
...
test('a dust output cannot receive a balance', () => {
  ...
  applyTx(s, {
    ...
    inputs: [{ txid: 'etch', vout: 0 }],
    outputs: [out(DUST - 1), out(DUST), opret(codec.encodeEdicts([{ runeRef: REF, amount: 5, output: 0 }]))],
  });
...
test('an edict cannot conjure more than the inputs carried', () => {
  ...
  applyTx(s, {
    ...
    inputs: [{ txid: 'etch', vout: 0 }],
    outputs: [out(), out(), opret(codec.encodeEdicts([{ runeRef: REF, amount: 999999999, output: 1 }]))],
  });
...
test('vector 7: a mint is refused past its close height and once the cap is reached', () => {
  ...
  assert.strictEqual(ok.runes.get(REF).mintCount, 2);
});

test('a mint cannot push the rune past its supply cap', () => {
...
test('a mint that underpays by one unit mints nothing', () => {
  ...
  assert.strictEqual(s.runes.get(REF).minted, 0);
});
...
test('a rune that charges nothing per mint does not need a fee at all', () => {
...
// in runes-tickers.test.js, where the lock itself is picked apart.
test('an etching that never locked the price does not take the ticker', () => {
  ...
  assert.strictEqual(s.tickers.get('FROG'), runeRefOf(101, 1));
});
...
test('re-spacing a name does not buy a second one', () => {
  ...
  assert.strictEqual(s.tickers.get('DOGGOTOTHEMOON'), runeRefOf(100, 1));
  ...
  assert.strictEqual(s.runes.size, 1, 'a separator bought a second rune');
  assert.strictEqual(s.tickers.size, 1);
  assert.strictEqual(s.tickers.get('DOGGOTOTHEMOON'), runeRefOf(100, 1), 'the first etcher lost the name');
});
...
test('the mask is normalised at indexing time, so every indexer draws the same name', () => {
  ...
  assert.strictEqual(s.runes.get(runeRefOf(100, 1)).spacers, 0b11);
});

// §6: an etch has no owner, so nothing about a registered rune can be revised after the fact. There
...
test('a field the protocol does not define is ignored, and the rune registers anyway', () => {
  const s = etched({ owner: 'DSOMEONE', mutable: true });
  const rune = s.runes.get(REF);
  assert.strictEqual(rune.ticker, 'FROG');
  assert.strictEqual(rune.owner, undefined);
...
test('indexing is deterministic: the same blocks always give the same state', () => {
  const txs = [
    ...
    { txid: 'm', height: 101, txIndex: 0, inputs: [{ txid: 'e', vout: 0 }],
      outputs: [out(), out(), opret(codec.encodeEdicts([{ runeRef: REF, amount: 40000, output: 1 }]))] },
  ];
