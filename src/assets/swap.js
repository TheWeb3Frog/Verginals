'use strict';
// Verge Assets: the rules that make an asset listing safe (ASSETS-SPEC-v0 §9).
//
// The inscription marketplace construction cannot be reused for a divisible asset unchanged, and the
...
//   asset it is not: the amount that leaves is decided by an edict the maker never signed.
//
...
// first eligible output, which is the buyer's. See test/assets-swap.test.js.
//
...
const CARRIER_INDEX = 1; // buyer: the new carrier the asset should land on
 ...
 *
 * @param {Object} carrierAssets { [assetRef]: amount } as read from the chain
 * @returns {{ sells: Array<{assetRef, amount}> }}
 */
function listingTerms(carrierAssets) {
  const entries = Object.entries(carrierAssets || {});
  if (entries.length === 0) throw new Error('this outpoint carries no asset, list it as an inscription instead');
  return {
    sells: entries
      .map(([assetRef, amount]) => ({ assetRef: Number(assetRef), amount: Number(amount) }))
      .sort((a, b) => a.assetRef - b.assetRef),
  };
}
...
function validateListing({ declared, onChain, partialAmount = null }) {
  ...
  if (c.length === 0) return { ok: false, reason: 'the carrier holds no asset on chain' };
  if (d.length !== c.length) return { ok: false, reason: 'the listing does not declare every asset on the carrier' };
  for (let i = 0; i < d.length; i++) {
    if (d[i][0] !== c[i][0]) return { ok: false, reason: `declared asset ${d[i][0]} is not on the carrier` };
    if (d[i][1] !== c[i][1]) {
      return { ok: false, reason: `declared ${d[i][1]} of asset ${d[i][0]}, chain says ${c[i][1]}` };
    }
  }
  return { ok: true };
}
...
function takerEdicts(declared) {
  return Object.entries(declared || {})
    .map(([assetRef]) => ({ assetRef: Number(assetRef), amount: 0, output: CARRIER_INDEX }))
    .sort((a, b) => a.assetRef - b.assetRef);
}
...
function verifySettlement({ state, txid, declared, buyerOutputs = [CARRIER_INDEX, PADDING_INDEX] }) {
  for (const [ref, amount] of Object.entries(declared || {})) {
    ...
    if (delivered !== Number(amount)) {
      return { ok: false, reason: `asset ${ref}: listed ${amount}, buyer received ${delivered}` };
    }
  }
  return { ok: true };
}
