...
const runeRefOf = (height, txIndex) => height * 1000 + txIndex;
...
function applyMint(state, tx, msg, pool) {
  ...
  if (rune.allowlistRoot && !allowlistOk(tx, rune, msg)) return;
 ...
 */
function allowlistOk(tx, rune, _msg) {
  ...
  return Buffer.isBuffer(rune.allowlistRoot) && node.equals(rune.allowlistRoot);
}
...
module.exports = { RuneState, applyTx, index, runeRefOf, outpoint };
