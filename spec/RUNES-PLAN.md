# Verge Runes: implementation plan

Companion to `RUNES-SPEC-v0.md`. Tracks what is built, what is next, and what blocks a mainnet
launch.
...
|---|---|---|---|
| 1 | Protocol spec | **done** | `spec/RUNES-SPEC-v0.md` |
| 1 | OP_RETURN codec (`src/runes/codec.js`) | **done** | 15 |
| 1 | State machine (`src/runes/indexer.js`) | **done** | 17 |
| 1 | Merkle checkpoints (`src/runes/checkpoint.js`) | **done** | 12 |
| 2 | Chain scanner + etch discovery (`src/runes/scanner.js`) | **done** | 11 |
| 2 | Transaction builders (`src/runes/builder.js`) | **done** | 14 |
| 2 | Rune-aware coin selection (`src/runes/coinselect.js`) | **done** | 13 |
...
the transaction carries a rune inscription. Reuses the existing `src/indexer.js` block reader.
...
**2.3 Wallet.** Rune-aware coin selection: never spend a rune-carrying UTXO to pay a fee. This is
...
3. Etch a rune, mint it, transfer it, publish a checkpoint, all as real broadcast transactions.
...
- the ticker schedule fixed and the Verge payout address confirmed by the Verge maintainers, since both are permanent once the first rune is etched;
- a public statement of what the protocol does **not** do (spec §10), so nobody builds on a promise
  the chain cannot keep.

The genesis etching should be deliberately unremarkable: a test rune with a tiny supply, so the
...
| Risk | Mitigation |
|---|---|
| A wallet spends a rune UTXO as fee change | rune-aware coin selection (2.3), the same guard inscriptions already need |
...
| A taker steals a partial rune sale | a listing sells the whole carrier, so a missing edict costs the maker nothing (§9.1); the attack itself is asserted in test/runes-swap.test.js |
