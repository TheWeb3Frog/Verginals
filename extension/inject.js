...
(function () {
  ...
  const api = {
    ...
    /**
     * Sign the transaction that reopens a name deposit, at etch time.
     *
     * Handled by the background and used by nothing for weeks, because it was never named here. The
     * whole point of the scheme is that after this signature the key becomes optional: the bytes get
     * inscribed, and in four years anybody can broadcast them and the money comes back to the etcher
     * even if they lost the wallet entirely. Unnamed on the provider, that promise was unreachable.
     */
    runesSignRelease: (opts) => call('runesSignRelease', opts),
