// Builders against a live regtest chain (ASSETS-PLAN §2.2 / §2.3).
//
// The unit tests prove the builders order outputs correctly in memory. This proves the ordering
// survives contact with a real node: two recipients, two different assets, broadcast for real, read
...
const { scanRange } = require(path.join(ROOT, 'src/assets/scanner'));
const { buildTransfer, buildMint, DUST_UNITS } = require(path.join(ROOT, 'src/assets/builder'));
const { selectCoins, assertNoAssetsSpent } = require(path.join(ROOT, 'src/assets/coinselect'));
 ...
 *
 *  1. `mustSpend` pins the asset-carrying outpoints as inputs. A transaction that does not spend the
...
(async () => {
  console.log('Verge Assets builders on regtest\n');

  const start = (await rpc('getblockchaininfo')).blocks;
  const REF = assetRefOf(start + 1, 1);
  const addr1 = await rpc('getnewaddress');
  const addr2 = await rpc('getnewaddress');

  // The asset, etched synthetically at the height we reference (etch-via-inscription is phase 2).
  ...
  // Both edicts name the SAME asset but different outputs, which is where an index mistake shows.
  //
  ...
  const carrierUtxo = { txid: mintTxid, vout: carrierVout, value: DUST_UNITS, assets: { [REF]: 2500 } };
  const selection = selectCoins({
    ...
    requiredAssets: [{ assetRef: REF, amount: 2500 }],
  });
  ...
  const transferPlan = buildTransfer([
    { address: addr1, value: DUST_UNITS, assets: [{ assetRef: REF, amount: 1000 }] },
    { address: addr2, value: DUST_UNITS, assets: [{ assetRef: REF, amount: 1500 }] },
  ]);
  ...
  // ---- 4. coin selection refuses to spend an asset utxo for a plain payment --------------------
  const utxos = [
    { txid: transferTxid, vout: 0, value: DUST_UNITS, assets: { [REF]: 1000 } }, // real carrier
    { txid: 'clean', vout: 0, value: 50 * DUST_UNITS },
  ];
  const plain = selectCoins({ utxos, targetValue: DUST_UNITS, fee: DUST_UNITS });
  check('a plain payment leaves the real asset-carrying utxo alone',
    !plain.inputs.some((u) => u.txid === transferTxid));
  let guarded = true;
  try { assertNoAssetsSpent(plain); } catch { guarded = false; }
