// The complete rune lifecycle on a live regtest chain (RUNES-PLAN phase 3, closing the loop).
//
// Everything before this stubbed the etching: the rune was injected into the state by hand. Here it
// is inscribed for real, in a commit + reveal pair, and the scanner has to FIND it on its own from
// the inscription envelope. Nothing about the rune is told to the indexer: it is discovered.
//
// Then a real mint and a real transfer are broadcast against that discovered rune, and the final
...
(async () => {
  console.log('Verge Runes full lifecycle on regtest\n');
  ...
  // ---- 2. the scanner DISCOVERS the rune with no help ----------------------------------------
  ...
  // Replay from blocks alone: nothing is injected, the rune must appear by itself.
  ...
  check('replaying the blocks registered the rune from scratch', state.tickers.has('CYCLE'),
    'tickers: ' + [...state.tickers.keys()].join(','));

  const REF = state.tickers.get('CYCLE');
  const premineHeld = [...state.entries()].filter((e) => e.runeRef === REF).reduce((s, e) => s + e.amount, 0);
  check('the premine landed on chain', premineHeld === 400000, 'held ' + premineHeld);

  // ---- 3. MINT against the discovered rune ----------------------------------------------------
  ...
  check('the rune tracked its issued total', state.runes.get(REF).minted === 5000);
  ...
  const tPlan = buildTransfer([
    { address: holder, value: DUST_UNITS, runes: [{ runeRef: REF, amount: 2000 }] },
    { address: buyer, value: DUST_UNITS, runes: [{ runeRef: REF, amount: 3000 }] }, // rune change
  ]);
  ...
  check('the rune change came back to the sender, not to the recipient',
  ...
  const total = [...state.entries()].filter((e) => e.runeRef === REF).reduce((s, e) => s + e.amount, 0);
  ...
  const anyEntry = [...state.entries()].find((e) => e.runeRef === REF);
  const proof = proveBalance(state, anyEntry.outpoint, anyEntry.runeRef);
