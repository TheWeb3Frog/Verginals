// The two prices of Verge Assets, on a real chain.
//
...
//   the mint price     a plain transaction FEE, set by whoever etched the asset, paid to the miner
...
//
// Run: RT_WALLET=<wallet> node test/e2e/assets-price-regtest.js
...
const { buildEtch, buildMint, DUST_UNITS } = require(path.join(ROOT, 'src/assets/builder'));
const scanner = require(path.join(ROOT, 'src/assets/scanner'));
const { AssetState, applyTx, assetRefOf } = require(path.join(ROOT, 'src/assets/indexer'));
const tickers = require(path.join(ROOT, 'src/assets/tickers'));
 ...
 */
async function mintPaying(assetRef, fee, recipient) {
  const plan = buildMint(assetRef, { address: recipient, value: DUST_UNITS, mintPrice: fee });
...
(async () => {
  ...
  console.log('The two prices of Verge Assets, on regtest\n');
  ...
  const REF = assetRefOf(etchHeight, 1);
  const asset = state.assets.get(REF);
  check('the indexer registered the asset from the chain alone', !!asset && asset.ticker === TICKER,
    asset ? asset.ticker + ', ' + scan.applied + ' transactions applied' : 'not registered');
  check('and it carries the mint price the etcher chose', !!asset && asset.terms.price === MINT_PRICE,
    asset && asset.terms ? xvg(asset.terms.price) + ' XVG per mint' : 'no terms');
  ...
  console.log('   which is why the builder caps a mint price at ' + xvg(tickers.MAX_MINT_PRICE)
    + ' XVG: above that, an ordinary wallet cannot mint the asset at all');
  ...
  const after = state.assets.get(REF);
  ...
  check('so the asset issued once for every mint that paid, and no others',
  ...
  const freeState = new AssetState();
  ...
  check('with the lock output taken away, the identical etching takes no ticker',
    freeState.assets.size === 0 && freeState.tickers.size === 0,
    freeState.assets.size + ' assets registered');
