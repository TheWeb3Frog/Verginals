...
/** Leaf = SHA256(outpoint || runeRef || amount), over a canonical text encoding of the triple. */
function leafHash({ outpoint, runeRef, amount }) {
  return sha256(Buffer.from(`${outpoint}|${runeRef}|${amount}`, 'utf8'));
}
...
/** The root committing to a whole RuneState. */
...
function proveBalance(state, outpoint, runeRef) {
  ...
  const idx = entries.findIndex((e) => e.outpoint === outpoint && e.runeRef === runeRef);
